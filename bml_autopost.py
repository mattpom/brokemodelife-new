...
if site["credit"]:
            credit_rule = ("Weave https://" + site["credit"] + " naturally into the post body as part of a sentence "
                                              "so X auto-links it. NEVER write 'from our friends at' or any variation.")
else:
...
# Post to Buffer using the current GraphQL API with variables (safe, no injection risk)
query = """
...
variables = {
            "input": {
                            "text": text,
                            "channelId": os.environ["PROFILE_ID"],
